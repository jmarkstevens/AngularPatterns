...
import {apiSetTreeView, apiSetTreeViewState} from '../api/api.Actions';

function _setTreeView(treeView) {
  apiSetTreeView(treeView);
}
...
function _saveTreeNew(_treeData, treeNode, location, tvState) {
  ...
  if (isNotRoot)
    ...
    _treeData = children;
  _setTreeView(_treeData);
  tvState.selected = newNode.nodeid;
  apiSetTreeViewState(tvState);
...
export default function handleActions(state = initialState, action) {
  ...
  switch (action.type) {
    case 'GetTreeViewStateDone':
      {
        if (action.payload.selected) {
          let tvData = _gotTreeViewState(treeCopy, action.payload);
          return {
            ...state,
            ...tvData
          }
